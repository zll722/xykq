  ...
  <div class="attendance-adjust-page">
    <section class="page-hero">
      <div>
        ...
        <h1>先找到记录，再决定如何修正</h1>
        <p>在页面里检索您的课程记录，再基于上下文完成补录或修正。</p>
      </div>
      <div class="hero-side">
...
import { listTeacherAttendanceRecordsApi, adjustTeacherAttendanceRecordApi, listTeacherClassesApi, listTeacherCoursesApi } from '../../api/teacherAttendanceManage';
...
const loadRecords = async () => {
  records.value = await listTeacherAttendanceRecordsApi({ ...filters });
};
...
const submit = async () => {
  ...
  try {
    await adjustTeacherAttendanceRecordApi(selected.value.id, {
