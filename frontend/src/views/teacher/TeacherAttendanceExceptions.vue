  ...
  <div class="attendance-exception-page">
    <section class="page-hero">
      <div>
        ...
        <p>先看今天哪些异常最值得优先处理，再按状态和日期筛选，最后带着上下文完成修正。</p>
      </div>
      <div class="hero-side">
      ...
      <el-form label-position="top">
        ...
        <el-form-item label="处理原因">
          <el-input
            ...
            maxlength="200"
            show-word-limit
            placeholder="例如：学生补交证明、确认设备故障导致漏签、审批通过后同步修正"
          />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { listTeacherAttendanceExceptionsApi, resolveTeacherAttendanceExceptionApi, listTeacherClassesApi, listTeacherCoursesApi } from '../../api/teacherAttendanceManage';
...
const loadData = async () => {
  ...
  try {
    ...
    list.value = decorate(await listTeacherAttendanceExceptionsApi(params));
  } catch (error) {
...
const goAdjust = (row) => {
  router.push({
    path: '/teacher/attendance-adjustments',
    query: {
...
const submit = async () => {
  ...
  try {
    await resolveTeacherAttendanceExceptionApi(selected.value.id, {
...
onMounted(() => {
  listTeacherClassesApi().then(res => classes.value = res);
  listTeacherCoursesApi().then(res => courses.value = res);
  loadData();
});
</script>
